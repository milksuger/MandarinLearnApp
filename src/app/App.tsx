...
import { Activity, ArrowLeft, ArrowRight, Ban, BookOpen, Check, ChevronLeft, ChevronRight, ChevronsLeft, ChevronsRight, CircleHelp, Clock3, Cloud, Compass, Flag, Flame, Headphones, Home, Layers3, LogOut, Menu, MessageCircle, Mic, Pause, PenLine, Play, RotateCcw, Search, Settings, ShieldCheck, SkipForward, Sparkles, Square, UserRound, Volume2, X } from "lucide-react";
import type HanziWriter from "hanzi-writer";
import { authClient } from "./auth-client";
import { ApiError, api, audioUrl } from "../shared/api";
...
function HomePage({ user }: { user: SessionUser | null }) {
  ...
  return <Protected><div className="home-wrap">
    ...
    {Boolean(dueReviews) && <Link className="daily-review-callout" to="/review"><span className="daily-review-icon"><RotateCcw /></span><span><strong>{dueReviews} materi siap diulang</strong><small>Luangkan beberapa menit agar kata yang sudah dipelajari lebih mudah diingat.</small></span><span className="button button-soft">Mulai ulasan</span></Link>}
...
function AdminCommunity() {
  ...
  const [activeQueue, setActiveQueue] = useState<"posts" | "comments" | "reports">("posts"); const [search, setSearch] = useState(""); const [page, setPage] = useState(0);
  const load = () => void api<typeof queue>("/admin/community/queue").then((data) => setQueue(data)).catch((cause) => setError(adminError(cause, "需要管理员权限才能查看此页面。")));
  useEffect(() => { load(); }, []);
  const moderate = async (kind: "posts" | "comments", id: string, decision: "approved" | "rejected") => { try { await api(`/admin/community/${kind}/${id}`, { method: "PATCH", body: JSON.stringify({ decision }) }); setNotice("审核决定已保存，并记入审计日志。"); load(); } catch (cause) { setError(adminError(cause, "审核决定未能保存，请重试。")); } };
  const reviewReport = async (reportId: string, hideContent: boolean) => { try { await api(`/admin/community-reports/${reportId}`, { method: "PATCH", body: JSON.stringify({ decision: hideContent ? "resolved" : "dismissed", hideContent }) }); setNotice("举报已处理。"); load(); } catch (cause) { setError(adminError(cause, "举报处理失败，请重试。")); } };
  const pageSize = 8;
  const posts = (queue?.posts ?? []).filter((item) => `${item.topicTitle ?? ""} ${item.authorName} ${item.body}`.toLowerCase().includes(search.trim().toLowerCase()));
  const comments = (queue?.comments ?? []).filter((item) => `${item.authorName} ${item.parentBody} ${item.body}`.toLowerCase().includes(search.trim().toLowerCase()));
  const reports = (queue?.reports ?? []).filter((item) => `${item.reason} ${item.details} ${item.body ?? ""}`.toLowerCase().includes(search.trim().toLowerCase()));
  const total = activeQueue === "posts" ? posts.length : activeQueue === "comments" ? comments.length : reports.length;
  const changeQueue = (next: typeof activeQueue) => { setActiveQueue(next); setPage(0); setSearch(""); setNotice(""); };
  return <><AdminPageHead eyebrow="内容管理" title="社区管理" subtitle="按队列审核帖子、回复或举报；单次只展开一个列表。管理员操作会记录在审计日志中。" />{error && <InlineNotice tone="danger">{error}</InlineNotice>}{notice && <InlineNotice>{notice}</InlineNotice>}
    <nav className="admin-content-tabs" aria-label="社区审核分类">{[{ id: "posts" as const, label: "待审核帖子", count: queue?.posts.length ?? 0 }, { id: "comments" as const, label: "待审核回复", count: queue?.comments.length ?? 0 }, { id: "reports" as const, label: "社区举报", count: queue?.reports.length ?? 0 }].map((tab) => <button key={tab.id} type="button" className={`admin-content-tab ${activeQueue === tab.id ? "active" : ""}`} aria-pressed={activeQueue === tab.id} onClick={() => changeQueue(tab.id)}><span>{tab.label}</span><b>{tab.count}</b></button>)}</nav>
    {queue && <section className="admin-panel review-section admin-queue-panel"><div className="panel-head"><div><h2>{activeQueue === "posts" ? "待审核帖子" : activeQueue === "comments" ? "待审核回复" : "社区举报"}</h2><p>搜索内容并逐条处理，避免在多个列表间反复滚动。</p></div><span className="queue-total">{total} 条待处理</span></div><label className="admin-queue-search"><Search /><input value={search} onChange={(event) => { setSearch(event.target.value); setPage(0); }} placeholder="搜索作者、主题或内容" /><span>{total} 条</span></label>
      {activeQueue === "posts" && (posts.length ? posts.slice(page * pageSize, (page + 1) * pageSize).map((post) => <div className="community-admin-item" key={post.id}><div><strong>{post.topicTitle ?? "未分类"} · {post.authorName}</strong><p>{post.body}</p><small>{new Date(post.createdAt).toLocaleString("zh-CN")}</small></div><div><button className="button button-soft" onClick={() => { setPage(0); void moderate("posts", post.id, "rejected"); }}>拒绝</button><button className="button button-primary" onClick={() => { setPage(0); void moderate("posts", post.id, "approved"); }}>通过</button></div></div>) : <div className="table-empty">没有待审核的帖子。</div>)}
      {activeQueue === "comments" && (comments.length ? comments.slice(page * pageSize, (page + 1) * pageSize).map((comment) => <div className="community-admin-item" key={comment.id}><div><strong>{comment.authorName} 回复：</strong><blockquote>{comment.parentBody}</blockquote><p>{comment.body}</p></div><div><button className="button button-soft" onClick={() => { setPage(0); void moderate("comments", comment.id, "rejected"); }}>拒绝</button><button className="button button-primary" onClick={() => { setPage(0); void moderate("comments", comment.id, "approved"); }}>通过</button></div></div>) : <div className="table-empty">没有待审核的回复。</div>)}
      {activeQueue === "reports" && (reports.length ? reports.slice(page * pageSize, (page + 1) * pageSize).map((report) => <div className="community-admin-item" key={report.id}><div><strong>{reportReasonZh(report.reason)} · {report.subjectType === "post" ? "帖子" : "回复"}</strong><p>{report.body ?? "内容已删除"}</p><small>{report.details || "没有补充说明"} · {new Date(report.createdAt).toLocaleString("zh-CN")}</small></div><div><button className="button button-soft" onClick={() => { setPage(0); void reviewReport(report.id, false); }}>关闭举报</button><button className="button button-danger" onClick={() => { setPage(0); void reviewReport(report.id, true); }}>隐藏内容</button></div></div>) : <div className="table-empty">没有未处理的举报。</div>)}
...
function GuidedWritingPage() {
  ...
  const returnTo = fromReview ? "/review" : returnWordId
...
function ReviewPage() {
  ...
  const [items, setItems] = useState<Array<{ contentType: string; contentId: string; dueAt: string; word: string | null; character: string | null; readingId: string | null; skill: string; prompt: string; pinyin: string | null; meaning: string | null; audioId: string | null }>>([]);
  ...
  useEffect(() => {
    let active = true; setLoading(true); setError(""); setRevealed(false); setRememberedCount(0); setRetryCount(0);
    void api<{ items: typeof items; scheduledCount: number; nextDueAt: string | null }>(`/reviews?skill=${skill === "all" ? "" : skill}`)
      .then((data) => { if (active) { setItems(data.items); setScheduledCount(data.scheduledCount); setNextDueAt(data.nextDueAt); } })
      .catch((cause) => { if (active) { setItems([]); setError(cause instanceof Error ? cause.message : "Belum dapat memuat jadwal ulasan."); } })
      .finally(() => { if (active) setLoading(false); });
    return () => { active = false; };
  }, [skill]);
  const activeItem = items[0];
  const submitReview = async (rating: "remembered" | "uncertain" | "repeat") => {
    if (!activeItem || saving) return;
    setSaving(true); setError("");
    try {
      await api("/reviews/answer", { method: "POST", body: JSON.stringify({ idempotencyKey: crypto.randomUUID(), contentType: activeItem.contentType, contentId: activeItem.contentId, readingId: activeItem.readingId, skill: activeItem.skill, rating }) });
      if (rating === "remembered") setRememberedCount((count) => count + 1); else setRetryCount((count) => count + 1);
      setItems((current) => current.slice(1)); setRevealed(false);
    } catch (cause) { setError(cause instanceof Error ? cause.message : "Jawaban belum tersimpan. Periksa koneksi lalu coba lagi."); }
  ...
  return <Protected><div className="page-wrap"><PageTitle eyebrow="PUSAT ULASAN" title="Ulangi yang sudah kamu pelajari" subtitle="Jadwal tiap keterampilan terpisah. Materi akan kembali sesuai kebutuhan, bukan sebagai tumpukan kartu kata." />
    ...
    {loading ? <div className="empty-card"><div className="loader" /><p>Memuat materi yang waktunya diulang…</p></div> : error && !activeItem ? <InlineNotice tone="danger">{error}</InlineNotice> : activeItem ? <section className="review-session-card" aria-live="polite">
      <div className="review-session-top"><span className="tag">{skillNames[activeItem.skill] ?? "ULANGI"}</span><span>{rememberedCount + retryCount + 1} dari {rememberedCount + retryCount + items.length}</span></div>
      <div className="review-session-target" lang="zh-Hans">{activeItem.prompt}</div>
      {activeItem.skill === "writing" && activeItem.contentType === "character" ? <div className="review-session-answer review-writing-action"><span className="tag">TANTANGAN MENULIS</span><strong>{activeItem.meaning ?? "Coba ingat bentuk karakternya"}</strong><p>Coba tulis karakter ini dari ingatan. Latihan berikutnya akan memeriksa urutan dan arah guratan.</p><Link className="button button-primary" to={`/write/${encodeURIComponent(activeItem.contentId)}?from=review`}>Mulai latihan guratan <ArrowRight /></Link></div> : <>
        {activeItem.pinyin && <p className="review-session-pinyin">{activeItem.pinyin}</p>}
        <AudioButton assetId={activeItem.audioId} text={activeItem.prompt} prominent />
        {!revealed ? <div className="review-session-recall"><p>Sebelum membuka jawabannya, coba ingat sendiri arti dan cara menggunakan materi ini.</p><button className="button button-primary" onClick={() => setRevealed(true)}>Lihat jawaban</button></div> : <div className="review-session-answer"><span className="tag">ARTI</span><strong>{activeItem.meaning ?? "Arti belum tersedia"}</strong><p>Seberapa mudah kamu mengingatnya?</p><div className="review-rating-actions"><button className="button button-soft" disabled={saving} onClick={() => void submitReview("repeat")}>Sulit · ulangi 10 menit lagi</button><button className="button button-soft" disabled={saving} onClick={() => void submitReview("uncertain")}>Belum yakin</button><button className="button button-primary" disabled={saving} onClick={() => void submitReview("remembered")}>{saving ? "Menyimpan…" : "Ingat dengan mudah"}</button></div></div>}
      </>}
      {error && <InlineNotice tone="danger">{error}</InlineNotice>}
      <div className="review-session-progress"><span style={{ width: `${((rememberedCount + retryCount) / (rememberedCount + retryCount + items.length)) * 100}%` }} /></div>
      <small className="review-session-footnote">Jawaban dan jadwal berikutnya tersimpan di akunmu dan mengikuti perangkat lain.</small>
    </section> : rememberedCount + retryCount > 0 ? <div className="lesson-result"><div className="lesson-result-mark"><Check /></div><span className="tag">ULANGAN SELESAI</span><h2>Sesi ulasan selesai.</h2><p>{rememberedCount} materi mudah diingat · {retryCount} materi dijadwalkan ulang. Materi yang sulit akan kembali lebih cepat.</p><div className="lesson-result-actions"><Link className="button button-primary" to="/">Kembali ke beranda</Link><Link className="button button-soft" to="/paths">Lanjutkan kursus</Link></div></div> : <div className="empty-card"><div className="empty-icon"><RotateCcw /></div><h3>{scheduledCount ? "Tidak ada yang perlu diulang sekarang" : "Jadwal ulasanmu akan muncul di sini"}</h3><p>{scheduledCount && nextDueAt ? `Ulasan berikutnya ${new Date(nextDueAt).toLocaleString("id-ID", { dateStyle: "medium", timeStyle: "short" })}. Setelah sesi belajar, materi akan masuk ke jadwal ini.` : "Selesaikan pelajaran atau latihan untuk mulai membangun jadwal ulasan pribadimu."}</p><Link to="/paths" className="button button-primary">{scheduledCount ? "Belajar materi baru" : "Pilih pelajaran"}</Link></div>}</div></Protected>;
}
...
function AuthPage({ mode, redirectTo = "/" }: { mode: "login" | "signup"; redirectTo?: string }) {
  const adminChinese = redirectTo.startsWith("/admin");
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    } catch (cause) { setError(adminChinese ? "登录失败，请检查邮箱、密码和网络连接后重试。" : cause instanceof Error ? cause.message : "Periksa koneksi lalu coba lagi."); }
    finally { setBusy(false); }
  };
  const beginRecovery = async (event: React.FormEvent) => {
    ...
    } catch (cause) { setError(adminChinese ? "邮箱或恢复码不匹配。" : cause instanceof Error ? cause.message : "Email atau kode pemulihan tidak cocok."); }
    finally { setBusy(false); }
  };
  const finishRecovery = async (event: React.FormEvent) => {
    ...
    try {
      ...
      setNoticeRecovery(adminChinese ? "密码已更新，请重新登录。" : "Kata sandi diperbarui. Silakan masuk kembali."); setRecoveryOpen(false); setResetToken("");
    } catch (cause) { setError(adminChinese ? "密码更新失败，请检查恢复码和网络后重试。" : cause instanceof Error ? cause.message : "Kata sandi belum dapat diperbarui."); }
    finally { setBusy(false); }
  };
  const [noticeRecovery, setNoticeRecovery] = useState("");
  if (generatedCodes.length) return <div className="auth-page"><div className="auth-card"><Link to="/" className="brand"><img className="brand-mark" src="/images/mandarin-learn-mascot.png" alt="" aria-hidden="true" /><span>{adminChinese ? "中文学习" : "Belajar Mandarin"}</span></Link><span className="tag">{adminChinese ? "重要安全步骤" : "LANGKAH KEAMANAN PENTING"}</span><h1>{adminChinese ? "保存恢复码" : "Simpan kode pemulihanmu"}</h1><p className="muted">{adminChinese ? "恢复码仅显示一次。请妥善保存，以便在没有付费邮件服务的情况下恢复账号。" : "Kode ini hanya ditampilkan sekali. Simpan di tempat aman agar akun tetap dapat dipulihkan tanpa email berbayar."}</p><div className="recovery-codes">{generatedCodes.map((code) => <code key={code}>{code}</code>)}<button className="button button-soft" onClick={() => void navigator.clipboard.writeText(generatedCodes.join("\n"))}>{adminChinese ? "复制全部恢复码" : "Salin kedua kode"}</button></div><button className="button button-primary button-wide" onClick={() => navigate("/")}>{adminChinese ? "我已保存" : "Saya sudah menyimpannya"} <ArrowRight /></button></div></div>;
  return <div className="auth-page"><div className="auth-card"><Link to="/" className="brand"><img className="brand-mark" src="/images/mandarin-learn-mascot.png" alt="" aria-hidden="true" /><span>{adminChinese ? "中文学习" : "Belajar Mandarin"}</span></Link><span className="tag">{adminChinese ? "管理员登录" : "BELAJAR BAHASA MANDARIN"}</span><h1>{adminChinese ? recoveryOpen ? "恢复管理员账号" : "管理员登录" : recoveryOpen ? "Pulihkan akun" : mode === "signup" ? "Mulai dari satu karakter." : "Senang melihatmu kembali."}</h1><p className="muted">{adminChinese ? recoveryOpen ? "输入邮箱和已保存的一组恢复码。" : "请使用管理员账号登录管理工作台。" : recoveryOpen ? "Masukkan email dan salah satu kode pemulihan yang kamu simpan." : "Akun memungkinkan kemajuanmu tersinkron di ponsel dan iPad."}</p>
    {noticeRecovery && <InlineNotice>{noticeRecovery}</InlineNotice>}
    {recoveryOpen ? <form onSubmit={(event) => void (resetToken ? finishRecovery(event) : beginRecovery(event))}><label>{adminChinese ? "邮箱" : "Email"}<input type="email" autoComplete="email" value={email} onChange={(event) => setEmail(event.target.value)} required maxLength={254} /></label>{!resetToken ? <label>{adminChinese ? "恢复码" : "Kode pemulihan"}<input autoComplete="one-time-code" value={recoveryCode} onChange={(event) => setRecoveryCode(event.target.value)} minLength={64} maxLength={64} required /></label> : <label>{adminChinese ? "新密码" : "Kata sandi baru"}<input type="password" autoComplete="new-password" value={newPassword} onChange={(event) => setNewPassword(event.target.value)} minLength={3} maxLength={128} required /><small>{adminChinese ? "密码至少需要 3 个字符。" : "Gunakan sedikitnya 3 karakter."}</small></label>}{error && <InlineNotice tone="danger">{error}</InlineNotice>}<button className="button button-primary button-wide" disabled={busy}>{busy ? adminChinese ? "处理中…" : "Memproses…" : resetToken ? adminChinese ? "保存新密码" : "Simpan kata sandi baru" : adminChinese ? "验证恢复码" : "Periksa kode"}</button><button type="button" className="button button-soft button-wide" onClick={() => { setRecoveryOpen(false); setError(""); setResetToken(""); }}>{adminChinese ? "返回登录" : "Kembali ke masuk"}</button></form> : <form onSubmit={(event) => void submit(event)}>{mode === "signup" && <label>Nama tampilan<input autoComplete="name" value={name} onChange={(event) => setName(event.target.value)} required maxLength={80} /></label>}<label>{adminChinese ? "邮箱" : "Email"}<input type="email" autoComplete="email" value={email} onChange={(event) => setEmail(event.target.value)} required maxLength={254} /></label><label>{adminChinese ? "密码" : "Kata sandi"}<input type="password" autoComplete={mode === "signup" ? "new-password" : "current-password"} value={password} onChange={(event) => setPassword(event.target.value)} minLength={3} maxLength={128} required />{mode === "signup" && <small>Gunakan sedikitnya 3 karakter.</small>}</label>{error && <InlineNotice tone="danger">{error}</InlineNotice>}<button className="button button-primary button-wide" disabled={busy}>{busy ? adminChinese ? "处理中…" : "Memproses…" : adminChinese ? "登录" : mode === "signup" ? "Buat akun" : "Masuk"} <ArrowRight /></button></form>}
    {!recoveryOpen && <p className="auth-switch">{adminChinese ? <><button className="text-button" onClick={() => { setRecoveryOpen(true); setError(""); }}>使用恢复码找回密码</button></> : mode === "signup" ? "Sudah punya akun?" : <>Lupa kata sandi? <button className="text-button" onClick={() => { setRecoveryOpen(true); setError(""); }}>Gunakan kode pemulihan</button></>} {!adminChinese && mode === "signup" && <Link to="/login">Masuk</Link>}{!adminChinese && mode === "login" && <> · <Link to="/signup">Daftar</Link></>}</p>}
    <small className="auth-privacy">{adminChinese ? "管理员登录不会使用付费邮件服务；重置密码需要账号恢复码。" : "Tanpa layanan email berbayar. Kode pemulihan diperlukan untuk mengatur ulang kata sandi."}</small></div></div>;
}
...
function AdminFrame({ user, isPending }: { user: SessionUser | null; isPending: boolean }) {
  ...
  return <div className="admin-frame"><aside className={`admin-sidebar ${open ? "sidebar-open" : ""}`}><Link className="brand" to="/admin"><img className="brand-mark" src="/images/mandarin-learn-mascot.png" alt="" aria-hidden="true" /><span>中文学习 <small>管理员</small></span></Link><span className="sidebar-label">管理工作台</span><NavLink to="/admin" end><Activity /> 数据概览</NavLink><NavLink to="/admin/learners"><UserRound /> 学习者</NavLink><NavLink to="/admin/content"><BookOpen /> 内容与音频</NavLink><NavLink to="/admin/community"><MessageCircle /> 社区管理</NavLink><NavLink to="/admin/audit"><ShieldCheck /> 审计与隐私</NavLink><div className="sidebar-spacer" /><Link to="/" className="sidebar-exit"><ArrowLeft /> 返回学习应用</Link><button className="sidebar-user" onClick={() => void authClient.signOut()}><span className="avatar">{data?.user.name.slice(0, 1).toUpperCase()}</span><span>{data?.user.name}<small>退出管理员</small></span><LogOut /></button></aside><main className="admin-content"><header className="admin-topbar"><button className="icon-button mobile-admin-menu" aria-label="打开菜单" onClick={() => setOpen((value) => !value)}><Menu /></button><span>管理工作台</span><span className="admin-account"><span className="status-dot" />已连接</span></header><div className="admin-page"><Outlet /></div></main>{open && <button className="sidebar-scrim" aria-label="关闭菜单" onClick={() => setOpen(false)} />}</div>;
}
...
function AdminOverview() {
  ...
  const [error, setError] = useState("");
  useEffect(() => { void api<{ metrics: typeof metrics }>("/admin/overview").then((data) => setMetrics(data.metrics)).catch((cause) => setError(adminError(cause, "需要管理员权限才能查看概览。"))); }, []);
  return <><AdminPageHead eyebrow="数据总览" title="管理概览" subtitle="查看账户活动和内容审核队列。数据来自服务器记录。" /><div className="admin-kpi-grid"><AdminKpi icon={<UserRound />} label="注册学习者" value={metrics?.learners ?? "—"} /><AdminKpi icon={<Activity />} label="近 7 天活跃" value={metrics?.activeLearnersLast7Days ?? "—"} /><AdminKpi icon={<BookOpen />} label="已记录练习" value={metrics?.attempts ?? "—"} /><AdminKpi icon={<Headphones />} label="待审核音频" value={metrics?.audioCandidates ?? "—"} /></div>{error && <InlineNotice tone="danger">{error}</InlineNotice>}<div className="admin-lower-grid"><div className="admin-panel"><div className="panel-head"><div><h2>内容审核队列</h2><p>审核通过前，内容不会展示给学习者。</p></div><Link className="text-link" to="/admin/content">查看队列 <ChevronRight /></Link></div><div className="queue-row"><span className="queue-dot queue-amber" /><span><strong>词汇内容</strong><small>等待来源核实与内容审核</small></span><b>{metrics?.vocabularyPendingReview ?? 0}</b></div><div className="queue-row"><span className="queue-dot queue-mint" /><span><strong>发音音频</strong><small>需与对应词语及语境一致</small></span><b>{metrics?.audioCandidates ?? 0}</b></div></div><div className="admin-panel"><div className="panel-head"><div><h2>学习者数据</h2><p>打开个人资料时会记录管理员访问日志。</p></div></div><Link className="button button-primary" to="/admin/learners">查找学习者 <ArrowRight /></Link></div></div></>;
}
...
function AdminLearners() {
  const [q, setQ] = useState(""); const [learners, setLearners] = useState<Array<{ id: string; name: string; masked_email: string; attempts: number; last_active: string | null }>>([]); const [error, setError] = useState(""); const [page, setPage] = useState(0);
  useEffect(() => { const timer = window.setTimeout(() => { void api<{ learners: typeof learners }>(`/admin/learners?q=${encodeURIComponent(q)}`).then((data) => { setLearners(data.learners); setPage(0); setError(""); }).catch((cause) => setError(adminError(cause, "学习者列表加载失败，请检查网络或权限。"))); }, 180); return () => window.clearTimeout(timer); }, [q]);
  const pageSize = 15; const visibleLearners = learners.slice(page * pageSize, (page + 1) * pageSize);
  return <><AdminPageHead eyebrow="学习者管理" title="学习者列表" subtitle="仅在需要时查看个人资料；每次访问都会记录在审计日志中。" /><label className="search-field"><Search /><input placeholder="搜索姓名或邮箱" value={q} onChange={(event) => { setQ(event.target.value); setPage(0); }} /></label>{error && <InlineNotice tone="danger">{error}</InlineNotice>}<div className="table-wrap"><table className="admin-table"><thead><tr><th>姓名</th><th>邮箱</th><th>练习次数</th><th>最近活跃</th><th></th></tr></thead><tbody>{visibleLearners.map((learner) => <tr key={learner.id}><td><Link to={`/admin/learners/${learner.id}`} className="learner-name"><span className="avatar avatar-small">{learner.name.slice(0, 1)}</span>{learner.name}</Link></td><td>{learner.masked_email}</td><td>{learner.attempts}</td><td>{learner.last_active ? new Date(learner.last_active).toLocaleDateString("zh-CN") : "尚未开始"}</td><td><Link className="text-link" to={`/admin/learners/${learner.id}`}>查看 <ChevronRight /></Link></td></tr>)}</tbody></table>{learners.length === 0 && <div className="table-empty">{error ? "暂时无法加载数据。" : "暂无学习者记录。"}</div>}</div><Pagination page={page} pageSize={pageSize} total={learners.length} onChange={setPage} /></>;
}
...
function AdminLearnerPage() {
  ...
  const [error, setError] = useState("");
  useEffect(() => { void api<typeof data>(`/admin/learners/${encodeURIComponent(learnerId)}`).then(setData).catch((cause) => setError(adminError(cause, "学习者资料暂不可用。"))); }, [learnerId]);
  return <><PageBack to="/admin/learners" /><AdminPageHead eyebrow="学习者资料" title={data?.learner.name ?? "学习者详情"} subtitle="此页面包含敏感信息，访问记录会自动写入审计日志。" />{error && <InlineNotice tone="danger">{error}</InlineNotice>}{data && <>
    <div className="learner-admin-hero"><div className="profile-avatar">{data.learner.name.slice(0, 1)}</div><div><strong>{data.learner.email}</strong><small>注册时间：{new Date(data.learner.createdAt).toLocaleDateString("zh-CN")} · 每日目标：{data.learner.daily_goal_minutes} 分钟</small></div></div>
    <div className="admin-kpi-grid"><AdminKpi icon={<BookOpen />} label="已学习内容" value={data.progress.length} /><AdminKpi icon={<Activity />} label="已记录练习" value={data.progress.reduce((sum, item) => sum + item.attempts, 0)} /><AdminKpi icon={<Clock3 />} label="记录的活跃天数" value={data.activity.length} /></div>
    <div className="admin-panel individual-progress"><div className="panel-head"><div><h2>按课程路径查看进度</h2><p>练习会关联到所属课程路径和单元。“通过”表示本次练习中所有已评估项目均达标。</p></div></div>{data.curriculumProgress.length ? data.curriculumProgress.map((item) => <div className="progress-row" key={`${item.curriculumName}-${item.unitTitle}`}><span className="unit-number">{item.levelNumber ?? "级"}</span><div><strong>{item.curriculumName} · {item.stageName ? `${item.stageName} · ` : ""}{item.unitTitle}</strong><small>{item.curriculumVersion} · {item.itemsPractised} 项内容 · {item.attempts} 次练习 · 最近学习：{item.lastSeenAt ? new Date(item.lastSeenAt).toLocaleDateString("zh-CN") : "暂无记录"}</small><small>{item.passed} 次通过 · {item.needsPractice} 次需加强 · {item.uncertain} 次不确定 · {item.notAssessed} 次未评估</small></div><span className="progress-score">{item.passed}/{item.attempts}</span></div>) : <div className="table-empty">暂无关联到课程单元的练习。</div>}</div>
    <div className="admin-panel individual-progress"><div className="panel-head"><div><h2>按技能查看进度</h2><p>听力、口语、阅读、书写、语法、词汇和理解能力分别统计；系统不保存原始录音。</p></div></div>{data.skillProgress.length ? <div className="skill-progress-grid">{data.skillProgress.map((item) => <article className="skill-progress-card" key={item.skill}><div><strong>{({ listening: "听力", speaking: "口语", reading: "阅读", writing: "书写", grammar: "语法", vocabulary: "词汇", comprehension: "理解" } as Record<string, string>)[item.skill] ?? item.skill}</strong><span>{item.itemsPractised} 项内容 · {item.attempts} 次练习</span></div><b>{item.passed}/{item.attempts}</b><small>{item.needsPractice} 次需复习 · {item.uncertain} 次不确定{item.lastSeenAt ? ` · ${new Date(item.lastSeenAt).toLocaleDateString("zh-CN")}` : ""}</small></article>)}</div> : <div className="table-empty">暂无按技能统计的练习结果。</div>}</div>
    <div className="admin-panel individual-progress"><div className="panel-head"><div><h2>课程活动</h2><p>每个阶段的学习进度均保存在服务器；书写进度显示已完成的汉字数量。</p></div></div>{data.activityProgress.length ? data.activityProgress.map((item) => <div className="progress-row" key={item.id}><span className={`activity-state activity-state-${item.state}`}>{item.state === "completed" ? <Check size={15} /> : item.state === "in_progress" ? <Clock3 size={15} /> : <BookOpen size={15} />}</span><div><strong>{item.curriculumName} · {item.unitTitle} · {item.title}</strong><small>{activityKindZh(item.activityKind)} · {item.state === "completed" ? item.activityKind === "comprehension" ? `已完成 · 答对 ${item.correctCount}/${item.itemCount}` : "已完成" : item.state === "in_progress" ? item.activityKind === "writing" ? `正在书写 · 已完成 ${item.writingCompletedCount}/${item.writingItemCount} 个汉字` : item.activityKind === "comprehension" ? `进行中 · 第 ${item.currentItemOrdinal + 1}/${item.itemCount} 题 · 答对 ${item.correctCount} 题` : `进行中 · 第 ${item.currentItemOrdinal + 1}${item.itemCount ? ` / 共 ${item.itemCount} 题` : " 题"}` : "尚未开始"}{item.updatedAt ? ` · ${new Date(item.updatedAt).toLocaleString("zh-CN")}` : ""}</small></div><span className="progress-score">{item.state === "completed" ? "✓" : item.activityKind === "writing" ? item.writingItemCount ? `${item.writingCompletedCount}/${item.writingItemCount}` : "—" : item.itemCount ? `${Math.min(item.currentItemOrdinal, item.itemCount)}/${item.itemCount}` : "—"}</span></div>) : <div className="table-empty">尚无已开始的课程活动。</div>}</div>
    <div className="admin-panel individual-progress"><div className="panel-head"><div><h2>起点水平评估</h2><p>最近一次由本应用生成的学习建议，并非官方 HSK 成绩。</p></div></div>{data.placements.length ? data.placements.map((item) => <div className="progress-row" key={item.id}><span className="activity-state"><Sparkles size={15} /></span><div><strong>{item.correct}/{item.answered} · {({ foundation: "建议从基础开始", elementary: "基础巩固与进阶", developing: "可以尝试进阶内容" } as Record<string, string>)[item.recommendation] ?? item.recommendation}</strong><small>{item.explanation} · {new Date(item.completedAt).toLocaleString("zh-CN")}</small></div></div>) : <div className="table-empty">暂无起点水平评估记录。</div>}</div>
    <div className="admin-panel individual-progress"><div className="panel-head"><div><h2>按内容查看进度</h2><p>系统保存练习结果摘要，不保存原始笔迹。</p></div></div>{data.progress.length ? data.progress.map((item) => <div className="progress-row" key={`${item.contentType}-${item.contentId}`}><span className="hanzi-thumb">{item.contentLabel?.slice(0, 1) ?? (item.contentId.startsWith("unicode:") ? item.contentId.slice(8, 9) : item.contentType === "character" ? "字" : "词")}</span><div><strong>{item.contentType === "character" ? "汉字" : "词汇"} · {item.contentLabel ?? item.contentId}</strong><small>{item.attempts} 次练习 · 最近学习：{item.lastSeenAt ? new Date(item.lastSeenAt).toLocaleDateString("zh-CN") : "暂无记录"}</small></div><span className="progress-score">{item.correct}/{item.attempts}</span></div>) : <div className="table-empty">暂无同步的学习记录。</div>}</div>
    <div className="admin-panel individual-progress"><div className="panel-head"><div><h2>手写识别确认记录</h2><p>仅记录学习者查看候选结果后确认的汉字，不保存原始笔迹。</p></div></div>{data.freehand.length ? data.freehand.map((item, index) => <div className="progress-row" key={`${item.codePoint}-${item.occurredAt}-${index}`}><span className="hanzi-thumb">{item.hanzi}</span><div><strong>{item.hanzi} · {item.codePoint}</strong><small>{item.engineId} · 候选排名 {item.candidateRank} · {new Date(item.occurredAt).toLocaleString("zh-CN")}</small></div></div>) : <div className="table-empty">暂无自由书写识别确认记录。</div>}</div>
  </>}</>;
}
...
function SentenceAudioRecordingStudio({ onUploaded }: { onUploaded: () => void }) {
  ...
  return <section className="admin-panel sentence-recording-studio">
    ...
    {!prompt ? <div className="recording-empty"><Headphones aria-hidden="true" /><strong>没有待录句子了。请检查下方审核队列中是否还有待试听的录音。</strong></div> : <div className="recording-workspace">
      <label className="recording-queue-picker"><span>选择待录句子 <small>{currentIndex + 1} / {prompts.length}</small></span><select value={currentIndex} disabled={recording || saving} onChange={(event) => { clearPreview(); setConsent(false); setExactConfirmed(false); setCurrentIndex(Number(event.target.value)); }} aria-label="选择待录句子">{prompts.map((item, index) => <option key={`${item.targetType}-${item.targetId}`} value={index}>{index + 1}. {item.unitTitle} · {item.text}</option>)}</select></label>
...
function AdminContent() {
  ...
  const [error, setError] = useState(""); const [notice, setNotice] = useState(""); const [audioReviewNotice, setAudioReviewNotice] = useState(""); const [reviewingAudioId, setReviewingAudioId] = useState<string | null>(null);
  const [activeTab, setActiveTab] = useState<"audio" | "recording" | "vocabulary" | "characters" | "readings">("audio");
  const [audioSearch, setAudioSearch] = useState(""); const [audioPage, setAudioPage] = useState(0);
  const load = () => { void api<typeof queue>("/admin/content/review-queue").then((data) => { setQueue(data); setError(""); }).catch((cause) => setError(adminError(cause, "内容队列加载失败，请检查网络或权限。"))); };
  useEffect(load, []);
  const review = async (kind: string, id: string, status: string) => { try { await api(`/admin/content/${kind}/${id}`, { method: "PATCH", body: JSON.stringify({ status, note: "管理员后台审核" }) }); setNotice("审核决定已保存，并记入审计日志。"); load(); } catch (cause) { setNotice(adminError(cause, "审核决定未能保存，请重试。")); } };
  const audioReview = async (id: string, decision: "passed" | "failed") => { setReviewingAudioId(id); setAudioReviewNotice(""); try { await api(`/admin/audio/${id}`, { method: "PATCH", body: JSON.stringify({ decision, note: "已对照页面显示的普通话文本和语境试听" }) }); setAudioReviewNotice(decision === "passed" ? "试听审核已通过，录音已从待审核列表移除。" : "已标记为不匹配，录音已从待审核列表移除。"); load(); } catch (cause) { setAudioReviewNotice(`审核未保存：${adminError(cause, "请检查网络或权限后重试。")}`); } finally { setReviewingAudioId(null); } };
  const tabs = [
    { id: "audio" as const, label: "试听审核", count: queue?.audio.length ?? 0 },
    { id: "recording" as const, label: "真人句子录音", count: null },
    { id: "vocabulary" as const, label: "词汇", count: queue?.vocabulary.length ?? 0 },
    { id: "characters" as const, label: "汉字与笔顺", count: queue?.characters.length ?? 0 },
    { id: "readings" as const, label: "读音与拼音", count: queue?.readings.length ?? 0 },
  ];
  const filteredAudio = (queue?.audio ?? []).filter((audio) => `${audio.text} ${audio.pinyin} ${audio.sourceName} ${audio.recordingContext}`.toLowerCase().includes(audioSearch.trim().toLowerCase()));
  const audioPageSize = 5;
  const visibleAudio = filteredAudio.slice(audioPage * audioPageSize, (audioPage + 1) * audioPageSize);
  const activateTab = (tab: typeof activeTab) => { setActiveTab(tab); setAudioPage(0); setAudioSearch(""); setNotice(""); setAudioReviewNotice(""); };
  return <>
    <AdminPageHead eyebrow="内容质量" title="内容与音频" subtitle="按类别分开处理；每次只显示一个审核队列，减少长页面滚动。" />
    {error && <InlineNotice tone="danger">{error}</InlineNotice>}{notice && <InlineNotice>{notice}</InlineNotice>}
    <nav className="admin-content-tabs" aria-label="内容审核分类">{tabs.map((tab) => <button key={tab.id} type="button" className={`admin-content-tab ${activeTab === tab.id ? "active" : ""}`} aria-pressed={activeTab === tab.id} onClick={() => activateTab(tab.id)}><span>{tab.label}</span>{tab.count !== null && <b>{tab.count}</b>}</button>)}</nav>
    {queue && activeTab === "audio" && <section className="admin-panel review-section admin-queue-panel"><div className="panel-head"><div><h2>真人音频待试听</h2><p>逐条核对文字、拼音和录音。通过或拒绝后，该条会从队列移除。</p></div><span className="queue-total">{filteredAudio.length} 条待处理</span></div>
      <label className="admin-queue-search"><Search /><input value={audioSearch} onChange={(event) => { setAudioSearch(event.target.value); setAudioPage(0); }} placeholder="搜索汉字、拼音、来源或语境" /><span>{filteredAudio.length} 条</span></label>
      {audioReviewNotice && <InlineNotice tone={audioReviewNotice.startsWith("审核未保存：") ? "danger" : "info"}>{audioReviewNotice}</InlineNotice>}
      {visibleAudio.length ? visibleAudio.map((audio) => <article className="audio-review-card" key={audio.id}><div className="audio-meta"><div className="audio-review-title"><strong>{audio.text}</strong><span>{audio.pinyin}</span><span className="audio-review-status">{audio.assetType === "sentence" ? "句子" : "词语"} · {audio.pronunciationReview === "pending" ? "待试听" : audio.pronunciationReview === "passed" ? "试听通过" : "试听未通过"}</span></div><audio controls preload="none" src={audioUrl(audio.id) ?? undefined} /><details className="audio-source-details"><summary>来源、授权与录音信息</summary><div><p>{audio.sourceName} · {audio.license} · {audio.sourceAttestedAt ? "来源已核实" : "来源待核实"}</p><p>{audio.recordingContext} · {audio.dialect} · {(audio.sizeBytes / 1024).toFixed(1)} KB · {audio.durationMs} 毫秒</p><small>校验码 {audio.sha256.slice(0, 16)}… · 署名：{audio.attribution}</small><p><a href={audio.sourcePageUrl} target="_blank" rel="noreferrer">查看来源</a> · <a href={audio.licenseUrl} target="_blank" rel="noreferrer">查看许可</a></p></div></details></div><div className="audio-review-actions"><button className="button button-soft" disabled={reviewingAudioId === audio.id} onClick={() => { setAudioPage(0); void audioReview(audio.id, "failed"); }}>{reviewingAudioId === audio.id ? "正在保存…" : "不匹配，拒绝"}</button><button className="button button-primary" disabled={reviewingAudioId === audio.id} onClick={() => { setAudioPage(0); void audioReview(audio.id, "passed"); }}>{reviewingAudioId === audio.id ? "正在保存…" : "试听后通过"}</button></div></article>) : <div className="table-empty">{audioSearch ? "没有匹配的录音。" : "暂无等待试听的录音。"}</div>}
      <Pagination page={audioPage} pageSize={audioPageSize} total={filteredAudio.length} onChange={setAudioPage} />
    </section>}
    {queue && activeTab === "recording" && <SentenceAudioRecordingStudio onUploaded={load} />}
    {queue && activeTab === "vocabulary" && <ReviewGroup key="vocabulary" title="词汇" items={queue.vocabulary} kind="vocabulary" onReview={review} />}
    {queue && activeTab === "characters" && <ReviewGroup key="characters" title="汉字与笔顺数据" items={queue.characters.map((item) => ({ ...item, text: `${item.text} · 笔顺：${strokeStatusZh(item.strokeStatus)}` }))} kind="character" onReview={review} />}
    {queue && activeTab === "readings" && <ReviewGroup key="readings" title="读音与拼音" items={queue.readings.map((item) => ({ ...item, text: `${item.text} · ${item.pinyin}` }))} kind="reading" onReview={review} />}
  </>;
}

function ReviewGroup({ title, items, kind, onReview }: { title: string; items: Array<{ id: string; text: string; status: string; sourceId: string }>; kind: string; onReview: (kind: string, id: string, status: string) => void }) {
  const [search, setSearch] = useState(""); const [page, setPage] = useState(0);
  const filtered = items.filter((item) => `${item.text} ${item.sourceId} ${item.status}`.toLowerCase().includes(search.trim().toLowerCase()));
  const pageSize = 10; const visible = filtered.slice(page * pageSize, (page + 1) * pageSize);
  return <section className="admin-panel review-section admin-queue-panel"><div className="panel-head"><div><h2>{title}</h2><p>可搜索内容，并逐条审核。审核后的条目会自动从队列移除。</p></div><span className="queue-total">{filtered.length} 项待审核</span></div>
    <label className="admin-queue-search"><Search /><input value={search} onChange={(event) => { setSearch(event.target.value); setPage(0); }} placeholder={`搜索${title}或来源`} /><span>{filtered.length} 项</span></label>
    {visible.length ? visible.map((item) => <div className="content-review-row" key={item.id}><span className="hanzi-thumb">{item.text.slice(0, 1)}</span><div><strong>{item.text}</strong><small>来源：{item.sourceId} · {contentStatusZh(item.status)}</small></div><div className="review-actions"><button className="button button-soft" onClick={() => { setPage(0); onReview(kind, item.id, "rejected"); }}>拒绝</button><button className="button button-primary" onClick={() => { setPage(0); onReview(kind, item.id, "approved"); }}>通过</button></div></div>) : <div className="table-empty">{search ? "没有匹配的内容。" : "当前队列没有待审核内容。"}</div>}
...
function Pagination({ page, pageSize, total, onChange }: { page: number; pageSize: number; total: number; onChange: (page: number) => void }) {
  ...
  return <div className="admin-pagination"><span>第 {page + 1} / {pageCount} 页 · 共 {total} 条</span><div><button type="button" className="button button-soft" disabled={page === 0} onClick={() => onChange(page - 1)}>上一页</button><button type="button" className="button button-soft" disabled={page + 1 >= pageCount} onClick={() => onChange(page + 1)}>下一页</button></div></div>;
}

function AdvancedPagination({ total, page, pageSize, onPageChange, onPageSizeChange }: { total: number; page: number; pageSize: number; onPageChange: (page: number) => void; onPageSizeChange: (pageSize: number) => void }) {
  ...
  const goToPage = () => {
    ...
    if (!Number.isFinite(requestedPage)) { setPageInput(String(page + 1)); return; }
    onPageChange(Math.min(pageCount - 1, Math.max(0, requestedPage - 1)));
  };
  return <nav className="advanced-pagination" aria-label="审计日志分页">
    <div className="advanced-page-summary">显示 <strong>{firstItem.toLocaleString("zh-CN")}–{lastItem.toLocaleString("zh-CN")}</strong> 条，共 <strong>{total.toLocaleString("zh-CN")}</strong> 条记录</div>
    <label className="page-size-control">每页显示<select value={pageSize} onChange={(event) => onPageSizeChange(Number(event.target.value))}><option value={10}>10 条</option><option value={25}>25 条</option><option value={50}>50 条</option><option value={100}>100 条</option></select></label>
    <div className="advanced-page-controls"><button type="button" className="button button-soft page-icon-button" aria-label="第一页" title="第一页" disabled={page <= 0} onClick={() => onPageChange(0)}><ChevronsLeft /></button><button type="button" className="button button-soft page-icon-button" aria-label="上一页" title="上一页" disabled={page <= 0} onClick={() => onPageChange(page - 1)}><ChevronLeft /></button><form className="page-jump-control" onSubmit={(event) => { event.preventDefault(); goToPage(); }}><label htmlFor="audit-page-number">第</label><input id="audit-page-number" type="number" min={1} max={pageCount} value={pageInput} onChange={(event) => setPageInput(event.target.value)} aria-label="跳转到第几页" /><span>页 / {pageCount}</span><button type="submit" className="button button-soft">跳转</button></form><button type="button" className="button button-soft page-icon-button" aria-label="下一页" title="下一页" disabled={page + 1 >= pageCount} onClick={() => onPageChange(page + 1)}><ChevronRight /></button><button type="button" className="button button-soft page-icon-button" aria-label="最后一页" title="最后一页" disabled={page + 1 >= pageCount} onClick={() => onPageChange(pageCount - 1)}><ChevronsRight /></button></div>
  </nav>;
}

function AdminAudit() {
  const [events, setEvents] = useState<Array<{ id: string; actorId: string | null; action: string; subjectType: string | null; subjectId: string | null; outcome: string; occurredAt: string }>>([]); const [total, setTotal] = useState(0); const [page, setPage] = useState(0); const [pageSize, setPageSize] = useState(25); const [error, setError] = useState("");
  const [requests, setRequests] = useState<Array<{ id: string; userId: string; name: string; maskedEmail: string; requestType: string; requestedAt: string }>>([]); const [privacyPage, setPrivacyPage] = useState(0); const [notice, setNotice] = useState("");
  const loadAudit = () => { void api<{ events: typeof events; total: number }>(`/admin/audit?limit=${pageSize}&offset=${page * pageSize}`).then((audit) => { setEvents(audit.events); setTotal(audit.total); setError(""); }).catch((cause) => setError(adminError(cause, "审计记录加载失败，请检查网络或权限。"))); };
  const loadRequests = () => { void api<{ requests: typeof requests }>("/admin/privacy-requests").then((data) => { setRequests(data.requests); setPrivacyPage(0); }).catch((cause) => setError(adminError(cause, "隐私请求加载失败，请检查网络或权限。"))); };
  useEffect(loadAudit, [page, pageSize]);
  useEffect(loadRequests, []);
  const deletionRequests = requests.filter((item) => item.requestType === "delete_account");
  const visibleDeletionRequests = deletionRequests.slice(privacyPage * 10, (privacyPage + 1) * 10);
  const handleRequest = async (request: typeof requests[number], decision: "approve_delete" | "reject") => {
    if (decision === "approve_delete" && !window.confirm(`确定删除 ${request.name} 的账号，以及已同步的学习进度和课程记录吗？此操作无法撤销。`)) return;
    try { await api(`/admin/privacy-requests/${request.id}`, { method: "PATCH", body: JSON.stringify({ decision }) }); setNotice(decision === "approve_delete" ? "账号及其已同步数据已删除。" : "此请求已标记为稍后复核。"); loadRequests(); loadAudit(); }
    catch (cause) { setNotice(adminError(cause, "请求处理失败，请稍后重试。")); }
  };
  return <><AdminPageHead eyebrow="审计与隐私" title="审计日志与隐私请求" subtitle="管理员访问和数据请求均有记录；审计日志仅供查看。" />{error && <InlineNotice tone="danger">{error}</InlineNotice>}{notice && <InlineNotice>{notice}</InlineNotice>}<section className="admin-panel review-section"><div className="panel-head"><div><h2>账号删除请求</h2><p>永久删除会一并删除服务器中保存的学习进度和课程记录。</p></div><span className="queue-total">{deletionRequests.length} 条待处理</span></div>{visibleDeletionRequests.length ? visibleDeletionRequests.map((request) => <div className="content-review-row" key={request.id}><span className="avatar avatar-small">{request.name.slice(0, 1)}</span><div><strong>{request.name} · {request.maskedEmail}</strong><small>申请时间：{new Date(request.requestedAt).toLocaleString("zh-CN")}</small></div><div className="review-actions"><button className="button button-soft" onClick={() => void handleRequest(request, "reject")}>拒绝</button><button className="button button-danger" onClick={() => void handleRequest(request, "approve_delete")}>删除数据</button></div></div>) : <div className="table-empty">没有待处理的账号删除请求。</div>}<Pagination page={privacyPage} pageSize={10} total={deletionRequests.length} onChange={setPrivacyPage} /></section><section className="admin-panel review-section audit-log-panel"><div className="panel-head"><div><h2>操作日志</h2><p>按发生时间倒序排列；切换页码只读取对应记录。</p></div><span className="queue-total">共 {total.toLocaleString("zh-CN")} 条</span></div><div className="table-wrap"><table className="admin-table"><thead><tr><th>时间</th><th>操作</th><th>数据类型</th><th>结果</th></tr></thead><tbody>{events.map((event) => <tr key={event.id}><td>{new Date(event.occurredAt).toLocaleString("zh-CN")}</td><td>{auditActionZh(event.action)}</td><td>{subjectTypeZh(event.subjectType)}</td><td><span className={`audit-outcome outcome-${event.outcome}`}>{outcomeZh(event.outcome)}</span></td></tr>)}</tbody></table>{events.length === 0 && <div className="table-empty">{error || "暂无审计记录。"}</div>}</div><AdvancedPagination total={total} page={page} pageSize={pageSize} onPageChange={setPage} onPageSizeChange={(size) => { setPageSize(size); setPage(0); }} /></section></>;
}
...
function adminError(cause: unknown, fallback: string) {
  if (cause instanceof ApiError) {
    const messages: Record<string, string> = {
      unauthorized: "登录状态已失效，请重新登录。", unauthenticated: "请先登录后再继续。", forbidden: "当前账号没有执行此操作的权限。",
      request_failed: "请求失败，请检查网络后重试。", rate_limited: "操作过于频繁，请稍后重试。",
      not_found: "请求的数据不存在或已被移除。", conflict: "数据已发生变化，请刷新页面后重试。",
      invalid_request: "提交的信息不完整或格式不正确。", invalid_decision: "当前无法执行此决定。", self_delete: "不能删除当前管理员账号。", last_owner: "删除前需要先指定另一位管理员。",
    };
    return messages[cause.code] ?? (cause.status === 401 ? "请先登录后再继续。" : cause.status === 403 ? "当前账号没有执行此操作的权限。" : fallback);
  }
  return fallback;
}

function contentStatusZh(status: string) { return ({ approved: "已通过", rejected: "已拒绝", candidate: "候选内容", draft: "草稿", pending: "待审核" } as Record<string, string>)[status] ?? status; }
function strokeStatusZh(status: string) { return ({ approved: "已核验", pending: "待核验", missing: "暂无数据", rejected: "未通过" } as Record<string, string>)[status] ?? status; }
function activityKindZh(kind: string) { return ({ listening: "听力", speaking: "口语", reading: "阅读", writing: "书写", grammar: "语法", vocabulary: "词汇", comprehension: "理解练习", dialogue: "情景对话", story: "短文阅读", pronunciation: "发音练习" } as Record<string, string>)[kind] ?? kind; }
function reportReasonZh(reason: string) { return ({ spam: "垃圾内容", harassment: "骚扰或攻击", personal_data: "个人信息", copyright: "版权问题", other: "其他原因" } as Record<string, string>)[reason] ?? reason; }
function subjectTypeZh(type: string | null) { return type ? ({ user: "用户", learner: "学习者", audio: "音频", audio_asset: "词语音频", content_audio_asset: "课程音频", vocabulary: "词汇", character: "汉字", community_post: "社区帖子", community_comment: "社区回复", post: "帖子", comment: "回复", privacy_request: "隐私请求" } as Record<string, string>)[type] ?? type : "—"; }
function auditActionZh(action: string) {
  const known: Record<string, string> = {
    "admin.bootstrap": "初始化管理员", "admin.audio.preview": "试听音频", "admin.audio.passed": "通过音频审核", "admin.audio.failed": "拒绝音频审核",
    "admin.content.approved": "通过内容审核", "admin.content.rejected": "拒绝内容审核", "admin.learner.read": "查看学习者资料", "admin.learners.list": "查看学习者列表",
    "admin.community.queue.read": "查看社区审核队列", "admin.community.approved": "通过社区内容", "admin.community.rejected": "拒绝社区内容",
    "admin.community.report.resolved": "处理社区举报", "admin.community.report.dismissed": "关闭社区举报", "admin.overview.read": "查看数据概览",
    "admin.audit.read": "查看审计记录", "admin.privacy.list": "查看隐私请求", "admin.privacy.delete_account": "删除学习者账号", "admin.privacy.rejected": "拒绝账号删除请求",
    "admin.sentence_audio.recorded": "提交真人录音", "privacy.export": "申请导出个人数据", "privacy.delete_account": "申请删除账号",
    "community.post.submit": "发布社区帖子", "community.comment.submit": "发布社区回复", "community.report.submit": "提交社区举报", "community.block": "屏蔽用户",
  };
  return known[action] ?? action;
}
function outcomeZh(outcome: string) { return ({ success: "成功", denied: "已拒绝", failure: "失败", failed: "失败", approved: "已通过", rejected: "已拒绝" } as Record<string, string>)[outcome] ?? outcome; }
