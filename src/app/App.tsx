...
function CreditsPage() {
  return <div className="page-wrap"><PageBack to="/profile" /><PageTitle eyebrow="KREDIT & LISENSI" title="Sumber materi dan teknologi" subtitle="Materi terbuka mempertahankan atribusi dan lisensinya sendiri; lisensi aplikasi tidak menggantikannya." />
    ...
    <section className="detail-card"><h2>Rekaman Mandarin</h2><p>Batch kosakata terbaru memuat 20 rekaman untuk kata yang sumbernya cocok; 36 kata lain tetap tanpa audio sampai tersedia rekaman yang tepat. Audio baru bersumber dari rekaman kata Lingua Libre yang ditandai sebagai Mandarin standar (<code>cmn</code>). Profil kontributor mengidentifikasi bahasa Mandarin sebagai bahasa ibu mereka. Sumbernya adalah kontribusi komunitas, bukan rekaman pemerintah atau sertifikasi fonetik resmi. WAV asli tetap diarsipkan; MP3 hanya ditranskode untuk kompatibilitas browser, tanpa mengedit ucapan. Setiap rekaman memiliki lisensi dan sumber masing-masing:</p>{audioAssetManifest.assets.map((asset) => <p key={asset.id}><strong>“{asset.text} / {asset.pinyin}”</strong> — {asset.creator}{asset.recordedBy ? `, direkam oleh ${asset.recordedBy}` : ""} · <a href={asset.sourcePage} target="_blank" rel="noreferrer">file sumber</a> · <a href={asset.licenseUrl} target="_blank" rel="noreferrer">{asset.license}</a>{asset.speakerProfile ? <> · <a href={asset.speakerProfile} target="_blank" rel="noreferrer">profil penutur</a></> : null}</p>)}</section>
