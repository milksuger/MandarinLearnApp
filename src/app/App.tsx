...
function AdminAudit() {
  ...
  const load = () => { void Promise.all([api<{ events: typeof events }>("/admin/audit"), api<{ requests: typeof requests }>("/admin/privacy-requests")]).then(([audit, privacy]) => { setEvents(audit.events); setRequests(privacy.requests); setError(""); }).catch((cause) => setError(adminError(cause, "审计记录加载失败，请检查网络或权限。"))); };
  useEffect(load, []);
  const handleRequest = async (request: typeof requests[number], decision: "approve_delete" | "reject") => {
    ...
    try { await api(`/admin/privacy-requests/${request.id}`, { method: "PATCH", body: JSON.stringify({ decision }) }); setNotice(decision === "approve_delete" ? "账号及其已同步数据已删除。" : "此请求已标记为稍后复核。"); load(); }
    catch (cause) { setNotice(adminError(cause, "请求处理失败，请稍后重试。")); }
  };
  return <><AdminPageHead eyebrow="审计与隐私" title="审计日志与隐私请求" subtitle="管理员访问和数据请求均有记录；审计日志仅供查看。" />{error && <InlineNotice tone="danger">{error}</InlineNotice>}{notice && <InlineNotice>{notice}</InlineNotice>}<section className="admin-panel review-section"><div className="panel-head"><div><h2>账号删除请求</h2><p>永久删除会一并删除服务器中保存的学习进度和课程记录。</p></div></div>{requests.length ? requests.filter((item) => item.requestType === "delete_account").map((request) => <div className="content-review-row" key={request.id}><span className="avatar avatar-small">{request.name.slice(0, 1)}</span><div><strong>{request.name} · {request.maskedEmail}</strong><small>申请时间：{new Date(request.requestedAt).toLocaleString("zh-CN")}</small></div><div className="review-actions"><button className="button button-soft" onClick={() => void handleRequest(request, "reject")}>拒绝</button><button className="button button-danger" onClick={() => void handleRequest(request, "approve_delete")}>删除数据</button></div></div>) : <div className="table-empty">没有待处理的账号删除请求。</div>}</section><div className="table-wrap"><table className="admin-table"><thead><tr><th>时间</th><th>操作</th><th>数据类型</th><th>结果</th></tr></thead><tbody>{events.map((event) => <tr key={event.id}><td>{new Date(event.occurredAt).toLocaleString("zh-CN")}</td><td>{auditActionZh(event.action)}</td><td>{subjectTypeZh(event.subjectType)}</td><td><span className={`audit-outcome outcome-${event.outcome}`}>{outcomeZh(event.outcome)}</span></td></tr>)}</tbody></table>{events.length === 0 && <div className="table-empty">{error || "暂无审计记录。"}</div>}</div></>;
}
