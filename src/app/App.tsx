...
function AudioButton({ assetId, text, prominent = false }: { assetId?: string | null; text: string; prominent?: boolean }) {
  const [error, setError] = useState("");
  ...
  const canSpeak = speechSupported && Boolean(text) && voiceAvailable !== false;
  return <span className="audio-control"><button className={`audio-button ${prominent ? "audio-button-prominent" : ""} ${!assetId && !canSpeak ? "audio-unavailable" : ""}`} disabled={!assetId && !canSpeak} onClick={() => void play()} aria-label={assetId ? "Putar rekaman Mandarin" : "Putar suara Mandarin lokal dari perangkat"} title={assetId ? "Putar rekaman Mandarin berlisensi" : canSpeak ? "Gunakan suara Mandarin lokal bawaan perangkat" : "Suara Mandarin lokal tidak tersedia di perangkat ini"}><Volume2 /></button><small>{error || (assetId ? (prominent ? "Dengarkan" : "Putar") : voiceAvailable === null && speechSupported ? "Cek suara Mandarin" : canSpeak ? "Suara perangkat" : "Audio tidak tersedia")}</small></span>;
}
