...
function AdminCommunity() {
  ...
  return <><AdminPageHead eyebrow="内容管理" title="社区管理" subtitle="新发布的帖子和回复在审核通过前仅作者可见。管理员操作会记录在审计日志中。" />{error && <InlineNotice tone="danger">{error}</InlineNotice>}{notice && <InlineNotice>{notice}</InlineNotice>}{queue && <>
    <section className="admin-panel review-section"><div className="panel-head"><div><h2>待审核帖子</h2><p>{queue.posts.length} 条帖子</p></div></div>{queue.posts.length ? queue.posts.map((post) => <div className="community-admin-item" key={post.id}><div><strong>{post.topicTitle ?? "未分类"} · {post.authorName}</strong><p>{post.body}</p><small>{new Date(post.createdAt).toLocaleString("zh-CN")}</small></div><div><button className="button button-soft" onClick={() => void moderate("posts", post.id, "rejected")}>拒绝</button><button className="button button-primary" onClick={() => void moderate("posts", post.id, "approved")}>通过</button></div></div>) : <div className="table-empty">没有待审核的帖子。</div>}</section>
    <section className="admin-panel review-section"><div className="panel-head"><div><h2>待审核回复</h2><p>{queue.comments.length} 条回复</p></div></div>{queue.comments.length ? queue.comments.map((comment) => <div className="community-admin-item" key={comment.id}><div><strong>{comment.authorName} 回复：</strong><blockquote>{comment.parentBody}</blockquote><p>{comment.body}</p></div><div><button className="button button-soft" onClick={() => void moderate("comments", comment.id, "rejected")}>拒绝</button><button className="button button-primary" onClick={() => void moderate("comments", comment.id, "approved")}>通过</button></div></div>) : <div className="table-empty">没有待审核的回复。</div>}</section>
    <section className="admin-panel review-section"><div className="panel-head"><div><h2>社区举报</h2><p>{queue.reports.length} 条未处理举报</p></div></div>{queue.reports.length ? queue.reports.map((report) => <div className="community-admin-item" key={report.id}><div><strong>{reportReasonZh(report.reason)} · {report.subjectType === "post" ? "帖子" : "回复"}</strong><p>{report.body ?? "内容已删除"}</p><small>{report.details || "没有补充说明"} · {new Date(report.createdAt).toLocaleString("zh-CN")}</small></div><div><button className="button button-soft" onClick={() => void reviewReport(report.id, false)}>关闭举报</button><button className="button button-danger" onClick={() => void reviewReport(report.id, true)}>隐藏内容</button></div></div>) : <div className="table-empty">没有未处理的举报。</div>}</section>
  </>}</>;
}
...
function AdminLearners() {
  ...
  useEffect(() => { const timer = window.setTimeout(() => { void api<{ learners: typeof learners }>(`/admin/learners?q=${encodeURIComponent(q)}`).then((data) => { setLearners(data.learners); setError(""); }).catch((cause) => setError(adminError(cause, "学习者列表加载失败，请检查网络或权限。"))); }, 180); return () => window.clearTimeout(timer); }, [q]);
  return <><AdminPageHead eyebrow="学习者管理" title="学习者列表" subtitle="仅在需要时查看个人资料；每次访问都会记录在审计日志中。" /><label className="search-field"><Search /><input placeholder="搜索姓名或邮箱" value={q} onChange={(event) => setQ(event.target.value)} /></label>{error && <InlineNotice tone="danger">{error}</InlineNotice>}<div className="table-wrap"><table className="admin-table"><thead><tr><th>姓名</th><th>邮箱</th><th>练习次数</th><th>最近活跃</th><th></th></tr></thead><tbody>{learners.map((learner) => <tr key={learner.id}><td><Link to={`/admin/learners/${learner.id}`} className="learner-name"><span className="avatar avatar-small">{learner.name.slice(0, 1)}</span>{learner.name}</Link></td><td>{learner.masked_email}</td><td>{learner.attempts}</td><td>{learner.last_active ? new Date(learner.last_active).toLocaleDateString("zh-CN") : "尚未开始"}</td><td><Link className="text-link" to={`/admin/learners/${learner.id}`}>查看 <ChevronRight /></Link></td></tr>)}</tbody></table>{learners.length === 0 && <div className="table-empty">{error ? "暂时无法加载数据。" : "暂无学习者记录。"}</div>}</div></>;
}
...
function AdminContent() {
  ...
  return <><AdminPageHead eyebrow="内容质量" title="内容与音频" subtitle="开放许可且来源与元数据匹配的内容可自动启用；此队列支持试听、审核通过或拒绝。" />{error && <InlineNotice tone="danger">{error}</InlineNotice>}{notice && <InlineNotice>{notice}</InlineNotice>}{queue && <><ReviewGroup title="词汇" items={queue.vocabulary} kind="vocabulary" onReview={review} /><ReviewGroup title="汉字与笔顺数据" items={queue.characters.map((item) => ({ ...item, text: `${item.text} · 笔顺：${strokeStatusZh(item.strokeStatus)}` }))} kind="character" onReview={review} /><ReviewGroup title="读音与拼音" items={queue.readings.map((item) => ({ ...item, text: `${item.text} · ${item.pinyin}` }))} kind="reading" onReview={review} />
    <SentenceAudioRecordingStudio onUploaded={load} />
    <section className="admin-panel review-section"><div className="panel-head"><div><h2>音频试听与审核</h2><p>逐句对照课程文本试听。确认是真人完整朗读且文字完全一致后，再通过审核。</p></div></div>{audioReviewNotice && <InlineNotice tone={audioReviewNotice.startsWith("审核未保存：") ? "danger" : "info"}>{audioReviewNotice}</InlineNotice>}{queue.audio.length ? queue.audio.map((audio) => <div className="audio-review-card" key={audio.id}><div className="audio-meta"><strong>{audio.text} <small>{audio.pinyin}</small></strong><p>{audio.assetType === "sentence" ? "句子录音" : "词语录音"} · {audio.sourceName} · {audio.license} {audio.sourceAttestedAt ? "· 来源已核实" : "· 等待审核"} · {audio.pronunciationReview === "pending" ? "待试听" : audio.pronunciationReview === "passed" ? "试听通过" : "试听未通过"}</p><p>{audio.recordingContext} · {audio.dialect} · {(audio.sizeBytes / 1024).toFixed(1)} KB · {audio.durationMs} 毫秒</p><small>校验码 {audio.sha256.slice(0, 16)}… · 署名：{audio.attribution}</small><p><a href={audio.sourcePageUrl} target="_blank" rel="noreferrer">查看来源</a> · <a href={audio.licenseUrl} target="_blank" rel="noreferrer">查看许可</a></p><audio controls preload="none" src={audioUrl(audio.id) ?? undefined} /></div><div className="audio-review-actions"><button className="button button-soft" disabled={reviewingAudioId === audio.id} onClick={() => void audioReview(audio.id, "failed")}>{reviewingAudioId === audio.id ? "正在保存…" : "不匹配，拒绝"}</button><button className="button button-primary" disabled={reviewingAudioId === audio.id} onClick={() => void audioReview(audio.id, "passed")}>{reviewingAudioId === audio.id ? "正在保存…" : "试听后通过"}</button></div></div>) : <div className="table-empty">暂无等待审核的录音。</div>}</section></>}</>;
}

function ReviewGroup({ title, items, kind, onReview }: { title: string; items: Array<{ id: string; text: string; status: string; sourceId: string }>; kind: string; onReview: (kind: string, id: string, status: string) => void }) { return <section className="admin-panel review-section"><div className="panel-head"><div><h2>{title}</h2><p>{items.length} 项待审核</p></div></div>{items.length ? items.map((item) => <div className="content-review-row" key={item.id}><span className="hanzi-thumb">{item.text.slice(0, 1)}</span><div><strong>{item.text}</strong><small>来源：{item.sourceId} · {contentStatusZh(item.status)}</small></div><div className="review-actions"><button className="button button-soft" onClick={() => onReview(kind, item.id, "rejected")}>拒绝</button><button className="button button-primary" onClick={() => onReview(kind, item.id, "approved")}>通过</button></div></div>) : <div className="table-empty">当前队列没有待审核内容。</div>}</section>; }
