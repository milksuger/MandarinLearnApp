...
function LessonPage() {
  ...
  const distractors = activeQuestion ? [...new Set(quizItems.filter((item) => item.placement_id !== activeQuestion.placement_id).map((item) => item.gloss).filter((gloss): gloss is string => Boolean(gloss)))].slice(0, 3) : [];
